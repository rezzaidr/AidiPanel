...
require "$INSTALLER" 'SHA256SUMS.sig' 'installer downloads a detached manifest signature'
require "$INSTALLER" '_release_manifest_verify' 'installer verifies the manifest before trusting hashes'
require "$INSTALLER" "IFS=' ' read -r checksum checksum_asset extra" 'installer parses signed checksums under strict global IFS'
reject "$INSTALLER" 'checksum-only' 'installer does not describe checksum-only verification as trusted'
