  ...
  <?php foreach ($sections as $s): ?>
    ...
    <?php if ($isSoon): ?>
      <div class="card p-4 flex items-start gap-3.5 opacity-70 select-none">
        <span class="w-10 h-10 rounded-lg <?= $tile ?> flex items-center justify-center shrink-0"><i class="ti <?= e($s['icon']) ?> text-xl"></i></span>
    ...
    <?php else: ?>
      <a href="<?= e($s['href']) ?>" class="card p-4 flex items-start gap-3.5 hover:border-ink/30 hover:shadow-sm transition group">
        ...
        <i class="ti ti-arrow-right text-zinc-300 group-hover:text-ink transition mt-1"></i>
      </a>
    <?php endif; ?>
  <?php endforeach; ?>
</div>
